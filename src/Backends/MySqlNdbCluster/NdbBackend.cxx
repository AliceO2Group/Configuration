...
///
/// NOTE: This class is super work in progress
///
...
#define FLPBENCH_MYSQL_DB_NAME "flpbench_db"
#define FLPBENCH_MYSQL_TABLE_NAME "benchmark_blob"
#define FLPBENCH_MYSQL_TABLE_KEY_NAME "ATTR1"
#define FLPBENCH_MYSQL_TABLE_KEY_LENGTH "255"
#define FLPBENCH_MYSQL_TABLE_VALUE_NAME "ATTR2"
...
int tableKeyLength()
{
  return boost::lexical_cast<int>(FLPBENCH_MYSQL_TABLE_KEY_LENGTH);
}
...
struct NdbPimpl
{
    NdbPimpl(std::string mySqlSocket, std::string clusterConnectString)
    {
      ndb_init();
...
//      cout << "clsConnStr : " << clusterConnectString << endl;
      ...
      mysql_query(mySql.get(), "CREATE DATABASE " FLPBENCH_MYSQL_DB_NAME);
      if (mysql_query(mySql.get(), "USE " FLPBENCH_MYSQL_DB_NAME) != 0) {
        throw std::runtime_error("MySQL error");
      }

      // Connect to database via NdApi
      myNdb.reset(new Ndb(clusterConnection.get(), FLPBENCH_MYSQL_DB_NAME));
      if (myNdb->init()) {
...
NdbBackend::NdbBackend(const std::string& host, int port)
    : mHost(host), mPort(port), ndbPimpl(new NdbPimpl(mySqlSocket, clusterConnectString))
{
}
...
{
  // Create table
  while (mysql_query(ndbPimpl->mySql.get(),
      "CREATE TABLE"
      "  " FLPBENCH_MYSQL_TABLE_NAME
      "    (" FLPBENCH_MYSQL_TABLE_KEY_NAME " VARCHAR(" FLPBENCH_MYSQL_TABLE_KEY_LENGTH ") NOT NULL PRIMARY KEY,"
      "     " FLPBENCH_MYSQL_TABLE_VALUE_NAME " TEXT NOT NULL)"
      "  ENGINE=NDB")) {
    if (mysql_errno(ndbPimpl->mySql.get()) == ER_TABLE_EXISTS_ERROR) {
//      cout << "MySQL Cluster already has table: " FLPBENCH_MYSQL_TABLE_NAME << endl;
      break;
    } else {
      throw std::runtime_error("MySQL error");
    }
  ...
  const NdbDictionary::Dictionary* myDict = ndbPimpl->myNdb->getDictionary();
  const NdbDictionary::Table* myTable = myDict->getTable(FLPBENCH_MYSQL_TABLE_NAME);

  for (auto& parameter : parameterMap) {
    NdbTransaction* myTransaction = ndbPimpl->myNdb->startTransaction();
    if (myTransaction == nullptr) {
      cout << "LINE: " << __LINE__ << endl;
      throw std::runtime_error(ndbPimpl->myNdb->getNdbError().message);
    }
    ...
    myNdbOperation->equal(FLPBENCH_MYSQL_TABLE_KEY_NAME, key.data(), key.length());

//    cout << "Putting key:" << key << endl;

    NdbBlob* myBlobHandle = myNdbOperation->getBlobHandle(FLPBENCH_MYSQL_TABLE_VALUE_NAME);
...
auto NdbBackend::getString(const std::string& path) -> Optional<std::string>
{
  ...
  if (!MYSQL_GET_USE_SINGLE_TRANSACTION) {
    ...
    for (auto& key : keys) {
      ...
      myNdbOperation->equal(FLPBENCH_MYSQL_TABLE_KEY_NAME, sqlKey.data(), sqlKey.length());
...
//      cout << replyValue << endl;
      ...
      ndbPimpl->myNdb->closeTransaction(myTransaction);
  ...
  //    cout << "CLOSED TRANSACTION" << endl;
    ...
    for (int i = 0; i < keyVector.size(); ++i) {
      ...
      myNdbOperations[i]->equal(FLPBENCH_MYSQL_TABLE_KEY_NAME, sqlKey.data(), sqlKey.length());

      myBlobHandles[i] = myNdbOperations[i]->getBlobHandle(FLPBENCH_MYSQL_TABLE_VALUE_NAME);
...
auto NdbBackend::getRecursive(const std::string& path) -> Tree::Node
{
  ...
  oss << "SELECT * FROM " FLPBENCH_MYSQL_TABLE_NAME
      << " WHERE " FLPBENCH_MYSQL_TABLE_KEY_NAME
      << " LIKE '" << key << "%'";

  if (mysql_query(ndbPimpl->mySql.get(), oss.str().c_str())) {
    throw std::runtime_error("MySQL query failed");
  }

  // TODO read data
  MYSQL_RES* result = mysql_store_result(ndbPimpl->mySql.get());
  if (result == nullptr) {
